...
@Component({
  ...
  template: `
    <div class="p-5 bg-yellow-50 border border-yellow-200 rounded-xl shadow-sm">
      ...
      <product-details-interactive-stars  [foundRate]="foundRate" (scoreChange)="onRate($event)"></product-details-interactive-stars>
...
export class SendRate {
  ...
  constructor(private rateService:RateService, private alertService: AlertService, private userService: UserService){}
  ...
  this.rateService.sendRate(this.productId, { score: this.score, comment: this.comment })
    .pipe(
...
updateRate() {
  ...
  this.rateService.updateRate(this.productId, this.foundRate.rateId.toString(), { score: this.score, comment: this.comment })
    .pipe(
      // finalize se ejecuta siempre al terminar el observable (success o error)
      finalize(() => {
        this.loading = false;
